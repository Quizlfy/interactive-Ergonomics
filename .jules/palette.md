...
## 2024-05-15 - Hide Decorative Elements from Screen Readers
**Learning:** In UI designs that frequently use emojis or initial-based avatars for visual enhancement (like the `.icon` and `.avatar` divs in this app), screen readers will confusingly read out the emoji names or arbitrary letters, creating unnecessary noise for visually impaired users.
**Action:** When implementing purely decorative visual elements, apply `aria-hidden="true"` to prevent redundant or noisy screen reader announcements and improve the clarity of the core content.
